...
export async function POST(request: globalThis.Request) {
  try {
    ...
    `);
    console.log("[SETUP] Dropped all existing tables");
    ...
    // Create KantoFundEntry table
    await db.$executeRawUnsafe(`
      CREATE TABLE "KantoFundEntry" (
        "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    ...
    await db.$executeRawUnsafe(`
      ...
      CREATE INDEX "ArtistProfile_storeStatus_idx" ON "ArtistProfile"("storeStatus");
    `);
    ...
    // Create Badge and UserBadge tables
    await db.$executeRawUnsafe(`
      CREATE TABLE "Badge" (
        "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    ...
    console.log("[SETUP] Seeded", defaultSettings.length, "site settings");

    return NextResponse.json({
      success: true,
      message: "Database initialized! Admin account and site settings created.",
